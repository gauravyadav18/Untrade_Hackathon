...
class Strategy:
    def run(self, data: pd.DataFrame, initial_balance=10000) -> pd.DataFrame:
        ...
        try:
            ...
            df['position'] = (0.00003 * initial_balance / df['atr']).clip(50, 100)  # Reduced position size
    ...
    def generate_ml_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        # Define target for ML model
        df['future_return'] = df['close'].pct_change().shift(-1)
        df['target'] = np.where((df['future_return'] > 0.0015) & (df['rsi'] > 50), 1,
                                np.where((df['future_return'] < -0.0015) & (df['rsi'] < 50), -1, 0))
    ...
    def apply_strategy(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data['atr_quantile'] = data['atr'].rolling(window=20).quantile(0.9)
        high_volatility = data['atr'] > data['atr_quantile']

        for i in range(1, len(data)):
            current_signal = data['ml_signal'].iloc[i]
            current_price = data['close'].iloc[i]
            current_date = data.index[i]
            is_high_volatility = high_volatility.iloc[i]
            ...
            if position_open:
                ...
                if position_type == TradeType.LONG.value:
                    trailing_sl = max(trailing_sl, current_price * (1 - 0.001))  # Tightened to 0.1%
                    ...
                    trailing_sl = min(trailing_sl, current_price * (1 + 0.001))  # Tightened to 0.1%
                    if current_price >= sl or current_price <= tp or current_price >= trailing_sl:
                        data.at[data.index[i], 'trade_type'] = TradeType.CLOSE.value
                        position_open = False
                        position_type = None
            ...
            # Entry logic with volatility filter
            if is_high_volatility:
                continue  # Skip trading during high volatility periods
            ...
            if current_signal == 1 and not position_open:
                ...
                sl = entry_price * (1 - 0.003)  # Tightened to 0.3%
                tp = entry_price * (1 + 0.002)  # Take-profit: 0.2%
                trailing_sl = entry_price * (1 - 0.001)  # Initial trailing stop at 0.1%
...
if __name__ == "__main__":
    ...
    print(result.tail())
# Output of this strategy has been shown in report.